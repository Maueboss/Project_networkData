...
ggraph(net_overall, layout = "stress", bbox = 15) +
  geom_edge_link2(aes(edge_colour = hwi), edge_linewidth = 0.5) +
  geom_node_point(aes(fill = sightings, size = degree), shape=21) +
  scale_fill_gradient(low = node_gradient[1], high = node_gradient[2]) + 
  geom_node_text(aes(label = id), vjust = 0.5, hjust = 0.5,
...
graph_F <- ggraph(net_overall, layout = 'matrix', sort.by = node_rank_spectral()) + 
